...
u8 txbuf[64] ={0x74,0x51,0xc0,0x7d,0x00,0x1d,0x00,0x08,
               0x01,0x02,0x03,0x04,0x05,0xef,0x03,0x01,
               0x07,0x33,0x34,0xa9,0x00,0x00,0x00,0x00,
  ...
  tep = spi_wr_data(0x5A,0xA5); // 开始读逻辑
	...
	for(i=0;i<64;i++)
	{
		...
		switch(i)
		{
			...
			break;
			case 0x11:
				DEBUG_SI24R2E_LOG( "低电压报警值：%02x\r\n",tep);
			break;
			case 0x13:
				DEBUG_SI24R2E_LOG( "低电压阈值：%d  \r\n",((tep & 0xF0)>>4)*5 + 190 ); break;
			break;
